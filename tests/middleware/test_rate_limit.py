...
def test_limiter_bounds_memory_under_fresh_flood(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A rotating-key flood cannot grow the bucket map past the cap."""
    import dyvine.middleware.rate_limit as module

    monkeypatch.setattr(module, "_MAX_TRACKED_KEYS", 3)
    limiter = TokenBucketLimiter(requests_per_second=1.0, burst_size=1)
    for index in range(10):
        limiter.allow(f"key-{index}", now=1000.0)
...
async def test_middleware_keys_on_validated_api_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Only the configured key earns its own bucket; rest share IP."""
    from dyvine.core.settings import settings

    monkeypatch.setattr(settings.security, "api_key", "alice")
    app = RateLimitMiddleware(_stub_app, requests_per_second=1.0, burst_size=1)
    alice = _scope(headers=[(b"x-api-key", b"alice")])
    assert (await _drain(app, alice))[0] == 200
    assert (await _drain(app, alice))[0] == 429
    # A wrong key falls back to the client-IP bucket, untouched so far.
    bob = _scope(headers=[(b"x-api-key", b"bob")])
    assert (await _drain(app, bob))[0] == 200
    assert (await _drain(app, bob))[0] == 429
    # No header shares that same IP bucket, now exhausted.
    assert (await _drain(app, _scope()))[0] == 429


async def test_middleware_key_rotation_does_not_reset_budget() -> None:
    """Rotating invalid keys shares one IP bucket (no evasion)."""
    app = RateLimitMiddleware(_stub_app, requests_per_second=1.0, burst_size=1)
    first = _scope(headers=[(b"x-api-key", b"rotated-1")])
    second = _scope(headers=[(b"x-api-key", b"rotated-2")])
    assert (await _drain(app, first))[0] == 200
    assert (await _drain(app, second))[0] == 429


async def test_middleware_prefers_forwarded_client_ip() -> None:
    """The rightmost X-Forwarded-For entry keys the IP bucket."""
    app = RateLimitMiddleware(_stub_app, requests_per_second=1.0, burst_size=1)
    spoofed = _scope(headers=[(b"x-forwarded-for", b"spoofed, 192.0.2.7")])
    direct = _scope(headers=[(b"x-forwarded-for", b"192.0.2.7")])
    assert (await _drain(app, spoofed))[0] == 200
    assert (await _drain(app, direct))[0] == 429
    # The direct peer has its own untouched bucket.
    assert (await _drain(app, _scope()))[0] == 200
...
def test_rate_limit_end_to_end_envelope() -> None:
    ...
    The limiter is registered before the correlation middleware (the
    production order in ``main.py``), so denials still pass back out
    through correlation and carry ``X-Correlation-ID``.
    """
    ...
    async def _correlation(request: Request, call_next):  # type: ignore[no-untyped-def]
        request.state.correlation_id = "corr-e2e"
        response = await call_next(request)
        response.headers["X-Correlation-ID"] = "corr-e2e"
        return response
